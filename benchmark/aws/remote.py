...
class Bench:
    ...
    def remove_loss(self, hosts=[]):
        assert isinstance(hosts, list)
        hosts = hosts if hosts else self.manager.hosts(flat=True)
        Print.info("Resetting loss on all nodes...")
        client_logs = [PathMaker.client_log_file(i) for i in range(len(hosts))]
        for host, log_file in zip(hosts, client_logs):
            cmd = CommandMaker.remove_loss()
            self._background_run(host, cmd, log_file)

    def add_loss(self, rate, sports, dports, ips, hosts=[]):
        assert isinstance(hosts, list)
        hosts = hosts if hosts else self.manager.hosts(flat=True)
        Print.info(
            f"Adding {rate} loss on sports {sports} and dports {dports} the following nodes..."
        )
        client_logs = [PathMaker.client_log_file(i) for i in range(len(hosts))]
        cmd = CommandMaker.add_loss(rate, sports, dports)
        for host, log_file in zip(hosts, client_logs):
