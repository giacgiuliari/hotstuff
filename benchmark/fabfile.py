...
@task
def remote(ctx, runs=2, duration=300):
    ...
    bench_params = {
        ...
        "tx_size": 512,
        "faults": 0,
        "duration": duration,  # 300 original
        "runs": runs,
    }
    node_params = {
        "consensus": {
...
@task
def add_loss(ctx, rate=0.0, ports="8000"):
    try:
        Bench(ctx).add_loss(rate, ports)
    except BenchError as e:
        Print.error(e)
